...
// Package miner implements Deltachain block creation and mining.
package miner
...
// Backend wraps all mdchods required for mining.
...
type Miner struct {
	...
	canStart    int32 // can start indicates whdeltachain we can start the mining operation
	shouldStart int32 // should start indicates whdeltachain we should start after sync
}

func New(dch Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		dch:      dch,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(config, engine, dch, mux, recommit, gasFloor, gasCeil, isLocalBlock),
		canStart: 1,
	}
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.SetDeltachainbase(coinbase)
...
// change between multiple mdchod calls
func (self *Miner) PendingBlock() *types.Block {
	return self.worker.pendingBlock()
}

func (self *Miner) SetDeltachainbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setDeltachainbase(addr)
}
