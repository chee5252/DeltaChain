...
func main() {
	...
	// Pre-generate the dchash mining DAG so we don't race
	dchash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".dchash"))
	...
	for _, node := range nodes {
		...
		if err := node.Service(&deltachaineum); err != nil {
			panic(err)
		}
		if err := deltachaineum.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var deltachaineum *dch.Deltachain
		if err := nodes[index%len(nodes)].Service(&deltachaineum); err != nil {
			panic(err)
		}
		...
		if err := deltachaineum.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := deltachaineum.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Deltachain node
	...
	// Start the node and configure a full Deltachain node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, err
	}
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return dch.New(ctx, &dch.Config{
			Genesis:         genesis,
			...
			GPO:             dch.DefaultConfig.GPO,
			Ethash:          dch.DefaultConfig.Ethash,
