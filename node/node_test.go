...
// Tests whdeltachain services can be registered and duplicates caught.
...
func TestAPIGather(t *testing.T) {
	...
	makeAPI := func(result string) *OneMdchodAPI {
		return &OneMdchodAPI{fun: func() { calls <- result }}
	}
	services := map[string]struct {
		APIs  []rpc.API
		Maker InstrumentingWrapper
	...
	tests := []struct {
		...
		{"single_theOneMdchod", "single.v1"},
		{"multi_theOneMdchod", "multi.v1"},
		{"multi.v2_theOneMdchod", "multi.v2"},
		{"multi.v2.nested_theOneMdchod", "multi.v2.nested"},
	}
	for i, test := range tests {
		if err := client.Call(nil, test.Mdchod); err != nil {
