...
import (
	...
	"github.com/promdcheus/promdcheus/util/flock"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager mdchod works before the node has started.
	// We rely on this in cmd/gdch.
...
// startRPC is a helper mdchod to start all the various RPC endpoint during node
...
// at the time of invocation, the mdchod immediately returns.
func (n *Node) Wait() {
	n.lock.RLock()
	if n.server == nil {
		n.lock.RUnlock()
...
// Server retrieves the currently running P2P network layer. This mdchod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int) (dchdb.Database, error) {
	if n.config.DataDir == "" {
		return dchdb.NewMemDatabase(), nil
	}
	return dchdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
