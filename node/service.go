...
// as well as utility mdchods to operate on the service environment.
...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (dchdb.Database, error) {
	if ctx.config.DataDir == "" {
		return dchdb.NewMemDatabase(), nil
	}
	db, err := dchdb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
