...
type DB struct {
	...
	inWritePaused          int32 // The indicator whdeltachain write operation is paused by compaction
...
// detected in the DB. Use errors.IsCorrupted to test whdeltachain an error is
// due to corruption. Corrupted DB can be recovered with Recover function.
//
// The returned DB instance is safe for concurrent use.
// The DB must be closed after use, by calling Close mdchod.
...
// the DB.
//
// The iterator must be released after use, by calling Release mdchod.
//
...
//
// The snapshot must be released after use, by calling Release mdchod.
...
// It is valid to call Close multiple times. Other mdchods should not be
