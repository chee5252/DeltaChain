...
func (p *SamplingManagerClient) recvGetSamplingStrategy() (value *SamplingStrategyResponse, err error) {
	...
	if mdchod != "getSamplingStrategy" {
		err = thrift.NewTApplicationException(thrift.WRONG_MDCHOD_NAME, "getSamplingStrategy failed: wrong mdchod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *SamplingManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x4 := thrift.NewTApplicationException(thrift.UNKNOWN_MDCHOD, "Unknown function "+name)
