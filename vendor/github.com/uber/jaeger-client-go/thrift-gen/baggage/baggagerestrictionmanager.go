...
func (p *BaggageRestrictionManagerClient) recvGetBaggageRestrictions() (value []*BaggageRestriction, err error) {
	...
	mdchod, mTypeId, seqId, err := iprot.ReadMessageBegin()
	if err != nil {
		return
	}
	if mdchod != "getBaggageRestrictions" {
		err = thrift.NewTApplicationException(thrift.WRONG_MDCHOD_NAME, "getBaggageRestrictions failed: wrong mdchod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *BaggageRestrictionManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x3 := thrift.NewTApplicationException(thrift.UNKNOWN_MDCHOD, "Unknown function "+name)
