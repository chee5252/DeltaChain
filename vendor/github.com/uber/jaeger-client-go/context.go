...
// IsSampled returns whdeltachain this trace was chosen for permanent storage
// by the sampling mechanism of the tracer.
func (c SpanContext) IsSampled() bool {
	return (c.flags & flagSampled) == flagSampled
}

// IsDebug indicates whdeltachain sampling was explicitly requested by the service.
func (c SpanContext) IsDebug() bool {
	return (c.flags & flagDebug) == flagDebug
}

// IsValid indicates whdeltachain this context actually represents a valid trace.
...
// used to return the debug/correlation ID from extract() mdchod. This happens
// in the situation when "jaeger-debug-id" header is passed in the carrier to
// the extract() mdchod, but the request otherwise has no span context in it.
// Previously this would've returned opentracing.ErrSpanContextNotFound from the
// extract mdchod, but now it returns a dummy context with only debugID filled in.
//
// See JaegerDebugHeader in constants.go
