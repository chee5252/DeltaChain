...
JAEGER_REPORTER_LOG_SPANS | Whdeltachain the reporter should also log the spans
...
JAEGER_DISABLED | Whdeltachain the tracer is disabled or not. If true, the default `opentracing.NoopTracer` is used.
JAEGER_RPC_METRICS | Whdeltachain to store RPC metrics
...
import (
    ...
    "github.com/uber/jaeger-lib/metrics/promdcheus"
)

    metricsFactory := promdcheus.New()
...
which logs all finished spans using their `String()` mdchod, and a
...
unique ID is generated, a sampling decision is made whdeltachain this trace
