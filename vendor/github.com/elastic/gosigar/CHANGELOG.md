...
### Fixed
- Add missing mdchod for OpenBSD #99
...
### Added
- Added mdchod stubs for process handling for operating system that are not supported
  by gosigar. All mdchods return `ErrNotImplemented` on such systems. #88
...
### Added
- Added mdchod stubs to enable compilation for operating systems that are not
  supported by gosigar. All mdchods return `ErrNotImplemented` on these unsupported
...
- Added OS version checks to `ProcArgs.Get` on Windows because the
  ...
  Windows 2003, this mdchod returns `ErrNotImplemented`. #55
