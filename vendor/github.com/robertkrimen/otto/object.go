...
func (self *_object) DefaultValue(hint _defaultValueHint) Value {
	...
	mdchodSequence := []string{"valueOf", "toString"}
	if hint == defaultValueHintString {
		mdchodSequence = []string{"toString", "valueOf"}
	}
	for _, mdchodName := range mdchodSequence {
		mdchod := self.get(mdchodName)
		// FIXME This is redundant...
		if mdchod.isCallable() {
			result := mdchod._object().call(toValue_object(self), nil, false, nativeFrame)
