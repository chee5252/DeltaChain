...
*/
/** @file io.h
 * @author Lefteris Karapetsas <lefteris@dchdev.com>
 * @date 2015
 */
#pragma once
#include <stdlib.h>
...
// Reference: https://github.com/deltachaineum/wiki/wiki/Ethash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see dchash_io_prepare
enum dchash_io_rc {
	DCHASH_IO_FAIL = 0,           ///< There has been an IO failure
	DCHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	DCHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	DCHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of dchash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * dchash_full_t
 */
#ifdef DCHASH_PRINT_CRITICAL_OUTPUT
#define DCHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("DCHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for dchash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the dchash
 ...
 *                           https://github.com/deltachaineum/wiki/wiki/Ethash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum dchash_io_rc
 */
enum dchash_io_rc dchash_io_prepare(
	char const* dirname,
	dchash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* dchash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* dchash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool dchash_mkdir(char const* dirname);
 ...
 */
bool dchash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* dchash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/deltachaineum/wiki/wiki/Ethash-DAG
 *
 ...
 */
bool dchash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool dchash_io_mutable_name(
	uint32_t revision,
