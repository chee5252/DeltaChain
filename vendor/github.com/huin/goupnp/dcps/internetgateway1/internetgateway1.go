...
const (
	...
	URN_WANDeltachainnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANDeltachainnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Deltachainnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Deltachainnet
//
...
// WANDeltachainnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANDeltachainnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANDeltachainnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANDeltachainnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANDeltachainnetLinkConfig1Clients() (clients []*WANDeltachainnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANDeltachainnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANDeltachainnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANDeltachainnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANDeltachainnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANDeltachainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANDeltachainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANDeltachainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANDeltachainnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANDeltachainnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANDeltachainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANDeltachainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANDeltachainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANDeltachainnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANDeltachainnetLinkConfig1 {
	clients := make([]*WANDeltachainnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANDeltachainnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewDeltachainnetLinkStatus: allowed values: Up, Down
func (client *WANDeltachainnetLinkConfig1) GetDeltachainnetLinkStatus() (NewDeltachainnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewDeltachainnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANDeltachainnetLinkConfig_1, "GetDeltachainnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewDeltachainnetLinkStatus, err = soap.UnmarshalString(response.NewDeltachainnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
