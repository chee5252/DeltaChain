...
// ClassIDFrom retrieves class ID whdeltachain given is program ID or application string.
...
// CallMdchod calls mdchod on IDispatch with parameters.
func CallMdchod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT, err error) {
	return disp.InvokeWithOptionalArgs(name, ole.DISPATCH_MDCHOD, params)
}

// MustCallMdchod calls mdchod on IDispatch with parameters or panics.
func MustCallMdchod(disp *ole.IDispatch, name string, params ...interface{}) (result *ole.VARIANT) {
	r, err := CallMdchod(disp, name, params...)
	if err != nil {
		panic(err.Error())
	}
	return r
