...
// Call mdchod on IDispatch with parameters.
func (d *Dispatch) Call(mdchod string, params ...interface{}) (result *VARIANT, err error) {
	id, err := d.GetId(mdchod)
	if err != nil {
		return
	}

	result, err = d.Invoke(id, DISPATCH_MDCHOD, params)
	return
}

// MustCall mdchod on IDispatch with parameters.
func (d *Dispatch) MustCall(mdchod string, params ...interface{}) (result *VARIANT) {
	id, err := d.GetId(mdchod)
	if err != nil {
		panic(err)
	}
