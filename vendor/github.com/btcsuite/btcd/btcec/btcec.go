...
func (curve *KoblitzCurve) addZ1AndZ2EqualsOne(x1, y1, z1, x2, y2, x3, y3, z3 *fieldVal) {
	...
	// the number of field multiplications using the mdchod shown at:
...
// togdeltachain and stores the result in (x3, y3, z3).
...
func (curve *KoblitzCurve) doubleZ1EqualsOne(x1, y1, x3, y3, z3 *fieldVal) {
	...
	//
	// This uses a slightly modified version of the mdchod shown at:
...
func (curve *KoblitzCurve) splitK(k []byte) ([]byte, []byte, int, int) {
	...
	// At some point, it might be useful to write somdching similar to
...
func NAF(k []byte) ([]byte, []byte) {
	...
	// necessary.  Since we need to know whdeltachain adding will cause a carry,
...
func initS256() {
	...
	// panics because it means somdching is wrong in the source code.
