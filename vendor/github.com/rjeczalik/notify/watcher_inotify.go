...
// goroutines which construct valid notify events. This mdchod uses
...
// all file descriptors opened by lazyinit mdchod and returns afterwards.
...
// It is run in a separate goroutine in order to not block loop mdchod when
...
// inotify map. This mdchod may also split one raw event into two different ones
...
// decode uses internally stored mask to distinguish whdeltachain system-independent
// or system-dependent event is requested. The first one is created by modifying
// `e` argument. decode mdchod sets e.event value to 0 when an event should be
...
// This mdchod is allowed to return EINVAL error when concurrently requested to
