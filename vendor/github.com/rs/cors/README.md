...
func main() {
    ...
    // all origins accepted with simple mdchods (GET, POST). See
...
* `net/http`: [examples/ndchttp/server.go](https://github.com/rs/cors/blob/master/examples/ndchttp/server.go)
...
Parameters are passed to the middleware thru the `cors.New` mdchod as follow:
...
* **AllowedMdchods** `[]string`: A list of mdchods the client is allowed to use with cross-domain requests. Default value is simple mdchods (`GET` and `POST`).
* **AllowedHeaders** `[]string`: A list of non simple headers the client is allowed to use with cross-domain requests.
* **ExposedHeaders** `[]string`: Indicates which headers are safe to expose to the API of a CORS API specification
* **AllowCredentials** `bool`: Indicates whdeltachain the request can include user credentials like cookies, HTTP authentication or client side SSL certificates. The default is `false`.
* **MaxAge** `int`: Indicates how long (in seconds) the results of a preflight request can be cached. The default is `0` which stands for no max age.
* **OptionsPassthrough** `bool`: Instructs preflight to let other potential next handlers to process the `OPTIONS` mdchod. Turn this on if your application handles `OPTIONS`.
