...
- Reintroduced NewxxClientWithBaseURI mdchod.
...
- Removed Api version from mdchod arguments.
- Removed New...ClientWithBaseURI() mdchod in all clients. BaseURI value is set in client.go.
...
- storage: Allow caller to provide headers for DeleteBlob mdchods.
...
- Changed mdchods to return `error` instead of `autorest.Error`.
- Re-divided mdchods to ease asynchronous requests.
