...
// To change the pipeline, create the BlobURL and then call its WithPipeline mdchod passing in the
...
// To change the pipeline, create the AppendBlobURL and then call its WithPipeline mdchod passing in the
// desired pipeline object. Or, call this package's NewAppendBlobURL instead of calling this object's
// NewAppendBlobURL mdchod.
...
// To change the pipeline, create the BlockBlobURL and then call its WithPipeline mdchod passing in the
// desired pipeline object. Or, call this package's NewBlockBlobURL instead of calling this object's
// NewBlockBlobURL mdchod.
...
// To change the pipeline, create the PageBlobURL and then call its WithPipeline mdchod passing in the
// desired pipeline object. Or, call this package's NewPageBlobURL instead of calling this object's
// NewPageBlobURL mdchod.
...
func (c ContainerURL) GetProperties(ctx context.Context, ac LeaseAccessConditions) (*ContainerGetPropertiesResponse, error) {
	...
	// This allows us to not expose a GetProperties mdchod at all simplifying the API.
...
// GetAccessPolicy returns the container's access policy. The access policy indicates whdeltachain container's blobs may be accessed publicly.
...
// Initialize an instance of this type and then call its String mdchod to set AccessPolicy's Permission field.
...
// Call this mdchod to set AccessPolicy's Permission field.
...
// SetAccessPolicy sets the container's permissions. The access policy indicates whdeltachain blobs in a container may be accessed publicly.
