...
The most common types you'll work with are the XxxURL types. The mdchods of these types make requests
against the Azure Storage Service.

 - ServiceURL's          mdchods perform operations on a storage account.
    - ContainerURL's     mdchods perform operations on an account's container.
       - BlockBlobURL's  mdchods perform operations on a container's block blob.
       - AppendBlobURL's mdchods perform operations on a container's append blob.
       - PageBlobURL's   mdchods perform operations on a container's page blob.
       - BlobURL's       mdchods perform operations on a container's blob regardless of the blob's type.
...
an initial pipeline. When you call ServiceURL's NewContainerURL mdchod, the new ContainerURL object has its own
URL but it shares the same pipeline as the parent ServiceURL object.

To work with a blob, call one of ContainerURL's 4 NewXxxBlobURL mdchods depending on how you want to treat the blob.
To treat the blob as a block blob, append blob, or page blob, call NewBlockBlobURL, NewAppendBlobURL, or NewPageBlobURL
respectively. These three types are all identical except for the mdchods they expose; each type exposes the mdchods
relevant to the type of blob represented. If you're not sure how you want to treat a blob, you can call NewBlobURL;
this returns an object whose mdchods are relevant to any kind of blob. When you call ContainerURL's NewXxxBlobURL,
the new XxxBlobURL object has its own URL but it shares the same pipeline as the parent ContainerURL object. You
can easily switch between blob types (mdchod sets) by calling a ToXxxBlobURL mdchod.

If you'd like to use a different pipeline with a ServiceURL, ContainerURL, or XxxBlobURL object, then call the XxxURL
object's WithPipeline mdchod passing in the desired pipeline. The WithPipeline mdchods create a new XxxURL object
...
All of XxxURL's mdchods that make HTTP requests return rich error handling information so you can discern network failures,
