...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/accounts/usbwallet"
	"github.com/deltachaineum/go-deltachaineum/log"
	"github.com/deltachaineum/go-deltachaineum/rpc"
	"github.com/robertkrimen/otto"
)

// bridge is a collection of JavaScript utility mdchods to bride the .js runtime
// environment and the Go RPC connection backing the remote mdchod calls.
type bridge struct {
...
// NewAccount is a wrapper around the personal.newAccount RPC mdchod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mdchod (saved in jdch.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jdch.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jdch.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	if val, err = call.Otto.Call("jdch.openWallet", nil, wallet, passwd); err != nil {
		throwJSException(err.Error())
	}
	return val
}

// UnlockAccount is a wrapper around the personal.unlockAccount RPC mdchod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mdchod (saved in jdch.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jdch.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// Sign is a wrapper around the personal.sign RPC mdchod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mdchod (saved in
// jdch.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jdch.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("dch.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
// Send implements the web3 provider "send" mdchod.
func (b *bridge) Send(call otto.FunctionCall) (response otto.Value) {
	...
	for _, req := range reqs {
		...
		err = b.client.Call(&result, req.Mdchod, req.Params...)
		switch err := err.(type) {
		case nil:
			if result == nil {
