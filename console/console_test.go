...
func newTester(t *testing.T, confOverride func(*dch.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Deltachain service within
	...
	dchConf := &dch.Config{
		...
		Deltachainbase: common.HexToAddress(testAddress),
		Ethash: dchash.Config{
			PowMode: dchash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(dchConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return dch.New(ctx, dchConf) }); err != nil {
		t.Fatalf("failed to register Deltachain protocol: %v", err)
	}
	...
	stack.Service(&deltachaineum)

	return &tester{
		workspace: workspace,
		stack:     stack,
		deltachaineum:  deltachaineum,
		console:   console,
		input:     prompter,
		output:    printer,
	}
