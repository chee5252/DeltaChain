...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Deltachain
...
type wallet struct {
	...
	deriveChain    deltachaineum.ChainStateReader // Blockchain state reader to discover used account with
...
// whdeltachain they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Deltachain account
			...
			// If the next account is empty, stop self-derivation, but add it nondcheless
...
// Contains implements accounts.Wallet, returning whdeltachain a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain deltachaineum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this mdchod will always return an error.
...
// data is not supported for Ledger wallets, so this mdchod will always return
// an error.
