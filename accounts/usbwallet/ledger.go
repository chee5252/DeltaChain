...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-dch/master/doc/dchapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Deltachain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Deltachain transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whdeltachain the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Deltachain app in browser mode", w.failure
	}
	if w.offline() {
		return "Deltachain app offline", w.failure
	}
	return fmt.Sprintf("Deltachain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whdeltachain the wallet and the Deltachain app is offline or not.
//
// The mdchod assumes that the state lock is held!
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Deltachain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Deltachain app's version, will fail prior to v1.0.2
...
// and returning the Deltachain address located on that derivation path.
...
//
// Note, if the version of the Deltachain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nondcheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Deltachain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Deltachain wallet app running
...
// ledgerDerive retrieves the currently active Deltachain address from a Ledger
...
//   Deltachain address length | 1 byte
//   Deltachain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Deltachain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Deltachain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whdeltachain legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the Deltachain signature and do a sanity validation
	if len(reply) != 65 {
