...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whdeltachain the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whdeltachain the Trezor is opened, closed
// or whdeltachain the Deltachain app was not started on it.
...
// Deltachain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.DeltachainAddress)
	if _, err := w.trezorExchange(&trezor.DeltachainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.DeltachainSignTx{
	...
	response := new(trezor.DeltachainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.DeltachainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mdchod will also return the index of the destination object used.
