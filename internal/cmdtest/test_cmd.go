...
type TestCmd struct {
	// For total convenience, all testing mdchods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gdch-test" in cmd/gdch/run_test.go)
...
// This mdchod can also be called from an expect template, e.g.:
//
//     gdch.expect(`Passphrase: {{.InputLine "password"}}`)
