...
func makeHeaderChain(parent *types.Header, n int, db dchdb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), dchash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (dchdb.Database, *LightChain, error) {
	db := dchdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db, indexerConfig: TestClientIndexerConfig}, gspec.Config, dchash.NewFaker())
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, dchash.NewFullFaker())
	if err != nil {
		panic(err)
	}
	return lc
...
func (odr *dummyOdr) Database() dchdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, dchash.NewFaker())
	if err != nil {
