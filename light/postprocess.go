...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/common/bitutil"
...
func GetChtRoot(db dchdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db dchdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    dchdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db dchdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := dchdb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, dchdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db dchdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db dchdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db dchdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := dchdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, dchdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
