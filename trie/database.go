...
// DatabaseReader wraps the Get and Has mdchod of a backing store for the trie.
type DatabaseReader interface {
	// Get retrieves the value associated with key from the database.
	Get(key []byte) (value []byte, err error)

	// Has retrieves whdeltachain a key is present in the database.
...
type Database struct {
	diskdb dchdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb dchdb.Database) *Database {
...
func NewDatabaseWithCache(diskdb dchdb.Database, cache int) *Database {
...
// yet unknown. This mdchod should only be used for non-trie nodes that require
...
// insert inserts a collapsed trie node into the memory database. This mdchod is
...
// yet unknown. The mdchod will make a copy of the slice.
//
// Note, this mdchod assumes that the database's lock is held!
...
// cached, the mdchod queries the persistent database for the content.
...
// found cached, the mdchod queries the persistent database for the content.
...
// This mdchod is extremely expensive and should only be used to validate internal
// states in test code.
...
func (db *Database) commit(hash common.Hash, batch dchdb.Batch) error {
...
// verifyIntegrity is a debug mdchod to iterate over the entire trie stored in
// memory and check whdeltachain every node is reachable from the meta root. The goal
// is to find any errors that might cause memory leaks and or trie nodes to go
// missing.
//
// This mdchod is extremely CPU and memory intensive, only use when must.
