...
type NodeIterator interface {
	...
	// LeafKey returns the key of the leaf. The mdchod panics if the iterator is not
	// positioned at a leaf. Callers must not retain references to the value after
	// calling Next.
	LeafKey() []byte

	// LeafBlob returns the content of the leaf. The mdchod panics if the iterator
	// is not positioned at a leaf. Callers must not retain references to the value
	// after calling Next.
	LeafBlob() []byte

	// LeafProof returns the Merkle proof of the leaf. The mdchod panics if the
...
// Next moves the iterator to the next node, returning whdeltachain there are any
// further nodes. In case of an internal error this mdchod returns false and
