...
func init() {
	spew.Config.Indent = "    "
	spew.Config.DisableMdchods = false
}

// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.Hash{}, NewDatabase(dchdb.NewMemDatabase()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(dchdb.NewMemDatabase()))
	if trie != nil {
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somdchingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(dchdb.NewMemDatabase())
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*dchdb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := dchdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
