...
type request struct {
	...
	raw  bool        // Whdeltachain this is a raw entry (code) or a trie node
...
func (s *Sync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togdeltachain
...
// as is. This mdchod's goal is to support misc state metadata retrievals (e.g.
// contract code).
...
// Process injects a batch of retrieved trie nodes data, returning if somdching
...
func (s *Sync) Commit(dbw dchdb.Putter) (int, error) {
...
func (s *Sync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whdeltachain known or not
	type child struct {
		node  node
		depth int
	}
