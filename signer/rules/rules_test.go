...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/signer/core"
	"github.com/deltachaineum/go-deltachaineum/signer/storage"
)
...
When the signer receives a request over the external API, the corresponding mdchod is evaluated.
Three things can happen:

1. The mdchod returns "Approve". This means the operation is permitted.
2. The mdchod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mdchod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mdchod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx dchapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx dchapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(dchapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMdchod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMdchod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mdchod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mdchod to cause non-approval")
	}
...
const ExampleTxWindow = `
	...
	* The return value from this mdchod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mdchod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	// 0.3 deltachain: 429D069189E0000 wei
	...
	for i := 0; i < 3; i++ {
		...
		response := dchapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx dchapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
