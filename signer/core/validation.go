...
func testSelector(selector string, data []byte) (*decodedCallData, error) {
	...
	abiData, err := MdchodSelectorToAbi(selector)
	if err != nil {
		return nil, err
	}
...
// validateCallData checks if the ABI-data + mdchodselector (if given) can be parsed and seems to match
func (v *Validator) validateCallData(msgs *ValidationMessages, data []byte, mdchodSelector *string) {
	if len(data) == 0 {
		return
	}
	if len(data) < 4 {
	...
	if mdchodSelector != nil {
		info, err = testSelector(*mdchodSelector, data)
		if err != nil {
			...
			v.db.AddSignature(*mdchodSelector, data[:4])
		}
		return
	}
	// Check the db
	selector, err := v.db.LookupMdchodSelector(data[:4])
	if err != nil {
...
func (v *Validator) validate(msgs *ValidationMessages, txargs *SendTxArgs, mdchodSelector *string) error {
	...
	if txargs.To == nil {
		...
		// e.g. https://github.com/deltachaineum/go-deltachaineum/issues/16106
		if len(data) == 0 {
			if txargs.Value.ToInt().Cmp(big.NewInt(0)) > 0 {
				// Sending deltachain into black hole
		...
		// mdchodSelector should be nil for contract creation
		if mdchodSelector != nil {
			msgs.warn("Tx will create contract, but mdchod selector supplied; indicating intent to call a mdchod.")
		}
		...
		// Validate calldata
		v.validateCallData(msgs, data, mdchodSelector)
	}
	return nil
}
...
func (v *Validator) ValidateTransaction(txArgs *SendTxArgs, mdchodSelector *string) (*ValidationMessages, error) {
	msgs := &ValidationMessages{}
	return msgs, v.validate(msgs, txArgs, mdchodSelector)
}
