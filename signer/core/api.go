...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mdchodSelector *string) (*dchapi.SignTransactionResult, error)
...
// SignerUI specifies what mdchod a UI needs to implement to be able to be used as a UI for the signer
type SignerUI interface {
	...
	// This mdchod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx dchapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mdchodSelector *string) (*dchapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(&args, mdchodSelector)
	if err != nil {
		return nil, err
	}
	...
	response := dchapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// OBS! This mdchod is removed from the public API. It should not be exposed on the external API
