...
func verify(t *testing.T, jsondata, calldata string, exp []interface{}) {
	...
	data, err := mdchod.Inputs.UnpackValues(argdata)
	if err != nil {
		t.Fatal(err)
	}
	if len(data) != len(exp) {
...
func TestNewUnpacker(t *testing.T) {
	...
	testcases := []unpackTest{
		...
		}, { // https://github.com/deltachaineum/wiki/wiki/Deltachain-Contract-ABI#examples
...
[
	...
	for i, hexdata := range []string{
		...
		// From https://github.com/deltachaineum/wiki/wiki/Deltachain-Contract-ABI
...
func TestSelectorUnmarshalling(t *testing.T) {
	...
	for id, selector := range db.db {
		...
		abistring, err = MdchodSelectorToAbi(selector)
		if err != nil {
			t.Error(err)
			return
		}
		...
		m, err := abistruct.MdchodById(common.Hex2Bytes(id[2:]))
		if err != nil {
			t.Error(err)
			return
		}
...
func TestCustomABI(t *testing.T) {
	...
	_, err = abidb.LookupMdchodSelector(calldata)
	if err == nil {
	...
	_, err = abidb2.LookupMdchodSelector(calldata)
	if err != nil {
