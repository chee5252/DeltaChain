...
func (ui *HeadlessUI) OnApprovedTx(tx dchapi.SignTransactionResult) {
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "dch-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *dchapi.SignTransactionResult
		err       error
	)
	...
	control <- "Y"
	control <- "wrongpassword"
	res, err = api.SignTransaction(context.Background(), tx, &mdchodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mdchodSig)
	if err != nil {
		t.Fatal(err)
	}
