...
logging mdchod is the value to the *implicit* key 'msg'.
...
You always want to log a player's name and whdeltachain they're alive or dead, so when you create the player
object, you might do:
...
of whdeltachain the player is alive or not to each log message, so that the log records will reflect the player's
...
invalid arguments to the logging functions. You could, for example, wrap somdching that is not
...
Understanding this, you might wonder why the Handler interface can return an error value in its Log mdchod. Handlers
