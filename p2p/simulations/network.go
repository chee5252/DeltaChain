...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togdeltachain.
//
...
// Connect connects two nodes togdeltachain by calling the "admin_addPeer" RPC
// mdchod on the "one" node so that it connects to the "other" node
...
// mdchod on the "one" node so that it disconnects from the "other" node
...
// NOTE:
// it also checks whdeltachain there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whdeltachain or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/deltachainsphere/go-deltachaineum/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whdeltachain or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whdeltachain both nodes are currently up
