...
creating a collection of devp2p nodes, connecting them togdeltachain to form a
...
if a node is created without an explicit service), exposes mdchods for
...
connecting two nodes togdeltachain).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somdching happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mdchod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whdeltachain or not a
...
As a concrete example, consider a simulated network of Deltachain nodes. An
...
On return, the `Simulation.Run` mdchod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mdchod> [<args>] [--subscribe]
```
