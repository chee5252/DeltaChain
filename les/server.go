...
func NewLesServer(dch *dch.Deltachain, config *dch.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(dch.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, dch.EventMux(), dch.Engine(), newPeerSet(), dch.BlockChain(), dch.TxPool(), dch.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(dch.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(dch.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(dch.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(dch.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(dch.ChainDb())
	return srv, nil
}
...
func newCostStats(db dchdb.Database) *requestCostStats {
