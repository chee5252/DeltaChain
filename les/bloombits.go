...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Deltachain
...
func (dch *LightDeltachain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-dch.shutdownChan:
					return

				case request := <-dch.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					compVectors, err := light.GetBloomBits(task.Context, dch.odr, task.Bit, task.Sections)
					if err == nil {
