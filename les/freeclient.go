...
func newFreeClientPool(db dchdb.Database, connectedLimit, totalLimit int, clock mclock.Clock) *freeClientPool {
	pool := &freeClientPool{
		db:             db,
		clock:          clock,
...
func (f *freeClientPool) connect(address string, disconnectFn func()) bool {
	...
	// check whdeltachain (linUsage+connectedBias) is smaller than the highest entry in the connected pool
