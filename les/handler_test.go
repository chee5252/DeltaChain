...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	case 1:
		...
		trie, _ := trie.New(root, trie.NewDatabase(dchdb.NewTable(server.db, light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(dchdb.NewTable(server.db, light.BloomTrieTablePrefix)))
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), dchash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), dchash.NewFaker(), db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
