...
type Vote struct {
	...
	Authorize bool           `json:"authorize"` // Whdeltachain to authorize or deauthorize the voted account
}

// Tally is a simple vote tally to keep the current score of votes. Votes that
// go against the proposal aren't counted since it's equivalent to not voting.
type Tally struct {
	Authorize bool `json:"authorize"` // Whdeltachain the vote is about authorizing or kicking someone
...
// mdchod does not initialize the set of recent signers, so only ever use if for
// the genesis block.
...
func loadSnapshot(config *params.CliqueConfig, sigcache *lru.ARCCache, db dchdb.Database, hash common.Hash) (*Snapshot, error) {
...
func (s *Snapshot) store(db dchdb.Database) error {
...
// validVote returns whdeltachain it makes sense to cast the specified vote in the
