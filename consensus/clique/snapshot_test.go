...
// mapped from textual names used in the tests below to actual Deltachain private
...
// address retrieves the Deltachain address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Deltachain address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/deltachaineum/go-deltachaineum/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gdch the header and prepare it for signing
			header := block.Header()
			if j > 0 {
