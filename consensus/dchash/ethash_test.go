...
// Tests that dchash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	dchash := NewTester(nil, false)
	defer dchash.Close()

	results := make(chan *types.Block)
	err := dchash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := dchash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/deltachaineum/go-deltachaineum/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "dchash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := dchash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	dchash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := dchash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	if _, err := api.GetWork(); err != errEthashStopped {
		t.Error("expect to return an error to indicate dchash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped dchash")
	}
}
