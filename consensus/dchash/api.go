...
var errEthashStopped = errors.New("dchash stopped")

// API exposes dchash related mdchods for the RPC interface.
type API struct {
	dchash *Ethash // Make sure the mode of dchash is normal.
}
...
func (api *API) GetWork() ([4]string, error) {
	if api.dchash.config.PowMode != ModeNormal && api.dchash.config.PowMode != ModeTest {
	...
	select {
	case api.dchash.fetchWorkCh <- &sealWork{errc: errc, res: workCh}:
	case <-api.dchash.exitCh:
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.dchash.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashRate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.dchash.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.dchash.exitCh:
		return false
	}
...
func (api *API) GetHashrate() uint64 {
	return uint64(api.dchash.Hashrate())
}
