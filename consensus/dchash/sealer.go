...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid dchash solution.
	staleThreshold = 7
)
...
func (dchash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if dchash.config.PowMode == ModeFake || dchash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", dchash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if dchash.shared != nil {
		return dchash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	dchash.lock.Lock()
	threads := dchash.threads
	if dchash.rand == nil {
		...
		dchash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	dchash.lock.Unlock()
	if threads == 0 {
	...
	if dchash.workCh != nil {
		dchash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			dchash.mine(block, id, nonce, abort, locals)
		}(i, uint64(dchash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", dchash.SealHash(block.Header()))
			}
			close(abort)
		case <-dchash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := dchash.Seal(chain, block, results, stop); err != nil {
...
func (dchash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = dchash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = dchash.dataset(number, false)
	)
	...
	logger.Trace("Started dchash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
...
func (dchash *Ethash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := dchash.SealHash(block.Header())
	...
	// whdeltachain the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := dchash.verifySeal(nil, header, true); err != nil {
	...
	for {
		...
		case result := <-dchash.submitWorkCh:
		...
		case errc := <-dchash.exitCh:
			// Exit remote loop if dchash is closed and return relevant error.
			errc <- nil
