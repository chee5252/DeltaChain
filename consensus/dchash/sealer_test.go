...
// Tests whdeltachain remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom dchash engine
	dchash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer dchash.Close()
	...
	dchash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := dchash.SealHash(header).Hex(); work[0] != want {
...
// Tests whdeltachain stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	dchash := NewTester(nil, true)
	defer dchash.Close()
	api := &API{dchash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			dchash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, dchash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch dchash result timeout", id+1)
		}
	}
}
