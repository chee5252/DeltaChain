...
#     not succeed, we fail.
#     + To prove an expression is zero, we check whdeltachain it belongs to the
#       ideal with the assumed zero expressions as basis. This test is exact.
#     + To prove an expression is nonzero, we check whdeltachain each of its
...
class fastfrac:
  ...
  def iszero(self,I):
    """Return whdeltachain this fraction is zero given an ideal."""
  ...
  def __rmul__(self,other):
    """Multiply somdching else with a fraction."""
...
def conflicts(R, con):
  """Check whdeltachain any of the passed non-zero assumptions is implied by the zero assumptions"""
  zero = R.ideal(map(numerator, con.zero))
  if 1 in zero:
    return True
  # First a cheap check whdeltachain any of the individual nonzero terms conflict on
  # their own.
...
def prove_nonzero(R, exprs, assume):
  """Check whdeltachain an expression is provably nonzero, given assumptions"""
  zero = R.ideal(map(numerator, assume.zero))
...
def prove_zero(R, exprs, assume):
  """Check whdeltachain all of the passed expressions are provably zero, given assumptions"""
