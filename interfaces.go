...
// Package deltachaineum defines interfaces for interacting with Deltachain.
package deltachaineum
...
// NotFound is returned by API mdchods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mdchods in this interface access raw
...
type ChainReader interface {
	...
	// This mdchod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type TransactionReader interface {
	...
	// blockchain. The isPending return value indicates whdeltachain the transaction has been
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mdchod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mdchod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by dch_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mdchod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mdchod can be used to retrieve the
