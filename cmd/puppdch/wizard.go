...
// config contains all the configurations needed by puppdch that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	dchstats  string   // Ethstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppdch configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Deltachain services known to be running on servers
...
// converts it to an Deltachain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by dchstats, which compares IPs textually, not by value.
