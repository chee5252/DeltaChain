...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.dchstats == "" {
		log.Error("No dchstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.dchashdir == "" {
			fmt.Printf("Where should the dchash mining DAGs be stored on the remote machine?\n")
			infos.dchashdir = w.readString()
		} else {
			fmt.Printf("Where should the dchash mining DAGs be stored on the remote machine? (default = %s)\n", infos.dchashdir)
			infos.dchashdir = w.readDefaultString(infos.dchashdir)
		}
	}
	...
	if infos.dchstats == "" {
		...
		infos.dchstats = w.readString() + ":" + w.conf.dchstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.dchstats)
		infos.dchstats = w.readDefaultString(infos.dchstats) + ":" + w.conf.dchstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an deltachainbase to mine against
			fmt.Println()
			if infos.deltachainbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.deltachainbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.deltachainbase)
				infos.deltachainbase = w.readDefaultAddress(common.HexToAddress(infos.deltachainbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Deltachain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
