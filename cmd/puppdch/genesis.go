...
import (
	...
	math2 "github.com/deltachaineum/go-deltachaineum/common/math"
...
// aldchGenesisSpec represents the genesis specification format used by the
// C++ Deltachain implementation.
type aldchGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*aldchGenesisSpecAccount `json:"accounts"`
}

// aldchGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type aldchGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *aldchGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// aldchGenesisSpecBuiltin is the precompiled contract definition.
type aldchGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *aldchGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type aldchGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAldchGenesisSpec converts a go-deltachaineum genesis block into a Aldch-specific
// chain specification format.
func newAldchGenesisSpec(network string, genesis *core.Genesis) (*aldchGenesisSpec, error) {
	// Only dchash is currently supported between go-deltachaineum and aldch
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Aldch format
	spec := &aldchGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(dchash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &aldchGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &aldchGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &aldchGenesisSpecBuiltin{Name: "sha256",
		Linear: &aldchGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &aldchGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &aldchGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &aldchGenesisSpecBuiltin{Name: "identity",
		Linear: &aldchGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &aldchGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &aldchGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &aldchGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &aldchGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &aldchGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &aldchGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *aldchGenesisSpec) setPrecompile(address byte, data *aldchGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*aldchGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &aldchGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *aldchGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*aldchGenesisSpecAccount)
	}
...
func (spec *aldchGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *aldchGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-deltachaineum genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only dchash is currently supported between go-deltachaineum and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.Ethash.Params.BlockReward["0x0"] = hexutil.EncodeBig(dchash.FrontierBlockReward)

	// Homestead
	spec.Engine.Ethash.Params.HomesteadTransition = hexutil.Uint64(genesis.Config.HomesteadBlock.Uint64())

	// Tangerine Whistle : 150
	// https://github.com/deltachaineum/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/deltachaineum/EIPs/blob/master/EIPS/eip-607.md
	...
	// gdch has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Deltachain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Deltachain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Deltachain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(dchash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(dchash.ConstantinopleBlockReward)
...
// pyDeltachainGenesisSpec represents the genesis specification format used by the
// Python Deltachain implementation.
type pyDeltachainGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyDeltachainGenesisSpec converts a go-deltachaineum genesis block into a Parity specific
// chain specification format.
func newPyDeltachainGenesisSpec(network string, genesis *core.Genesis) (*pyDeltachainGenesisSpec, error) {
	// Only dchash is currently supported between go-deltachaineum and pydeltachaineum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyDeltachainGenesisSpec{
