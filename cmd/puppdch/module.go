...
// returned from docker inspect, parsed into a form easily usable by puppdch.
type containerInfos struct {
	running bool              // Flag whdeltachain the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whdeltachain there's a container running for the service
	...
	// Infos retrieved, parse the above into somdching meaningful
