...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Deltachains to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.dchstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.dchstats = w.readString() + ":" + w.conf.dchstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.dchstats)
		infos.node.dchstats = w.readDefaultString(infos.node.dchstats) + ":" + w.conf.dchstats
	}
