...
FROM puppdch/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gdch --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gdch --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --dchstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppdchNetworkID/{{.NetworkID}}/g' dist/js/deltachainwallet-master.js && \
	sed -i 's/PuppdchNetwork/{{.Network}}/g'     dist/js/deltachainwallet-master.js && \
	sed -i 's/PuppdchDenom/{{.Denom}}/g'         dist/js/deltachainwallet-master.js && \
	sed -i 's/PuppdchHost/{{.Host}}/g'           dist/js/deltachainwallet-master.js && \
	sed -i 's/PuppdchRPCPort/{{.RPCPort}}/g'     dist/js/deltachainwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whdeltachain
// it's running, and if yes, whdeltachain it's responsive.
