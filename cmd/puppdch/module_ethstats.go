...
// dchstatsDockerfile is the Dockerfile required to build an dchstats backend
// and associated monitoring site.
var dchstatsDockerfile = `
FROM puppdch/dchstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// dchstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an dchstats monitoring site.
var dchstatsComposefile = `
version: '2'
services:
  dchstats:
    build: .
    image: {{.Network}}/dchstats
    container_name: {{.Network}}_dchstats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new dchstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(dchstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(dchstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the dchstats service
	if nocache {
...
// dchstatsInfos is returned from an dchstats status check to allow reporting
// various configuration parameters.
type dchstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *dchstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an dchstats server to verify whdeltachain
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*dchstatsInfos, error) {
	// Inspect a possible dchstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_dchstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
