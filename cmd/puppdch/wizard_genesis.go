...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of dchash, we're pretty much done
...
// importGenesis imports a Gdch genesis spec into puppdch.
...
func (w *wizard) manageGenesis() {
	// Figure out whdeltachain to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-aldch.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppdch and Gdch
		gdchJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gdchJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gdchJson)

		// Export the genesis spec used by Aldch (formerly C++ Deltachain)
		if spec, err := newAldchGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Aldch chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "aldch", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly DeltachainJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somdching I can do")
		return
	}
}
