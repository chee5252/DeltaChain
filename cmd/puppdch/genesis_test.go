...
// Tests the go-deltachaineum to Aldch chainspec conversion for the Stureby testnet.
func TestAldchSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gdch.json")
	if err != nil {
	...
	spec, err := newAldchGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_aldch.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &aldchGenesisSpec{}
...
// Tests the go-deltachaineum to Parity chainspec conversion for the Stureby testnet.
