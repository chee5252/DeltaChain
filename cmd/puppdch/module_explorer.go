...
FROM puppdch/explorer:latest

ADD dchstats.json /dchstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../dch-net-intelligence-api && pm2 start /dchstats.json)' >  explorer.sh && \
	echo '(cd ../deltachainchain-light && npm start &)'                      >> explorer.sh && \
	echo 'exec /parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEthstats is the configuration file for the dchstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.deltachaineum
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	dchstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(dchstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.dchstats[:strings.Index(config.dchstats, ":")],
		"Secret": config.dchstats[strings.Index(config.dchstats, ":")+1 : strings.Index(config.dchstats, "@")],
		"Host":   config.dchstats[strings.Index(config.dchstats, "@")+1:],
	})
	files[filepath.Join(workdir, "dchstats.json")] = dchstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.deltachaineum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		dchstats: infos.envvars["STATS"],
	}
	return stats, nil
}
