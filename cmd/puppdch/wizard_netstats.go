...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for dchstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["dchstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["dchstats"] = infos.Report()
		dchstats = infos.config
	}
