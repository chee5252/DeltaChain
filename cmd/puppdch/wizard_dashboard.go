...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"dchstats", "explorer", "wallet", "faucet"} {
	...
	// If we have dchstats running, ask whdeltachain to make the secret public or not
	if w.conf.dchstats != "" {
		fmt.Println()
		fmt.Println("Include dchstats secret on dashboard (y/n)? (default = yes)")
