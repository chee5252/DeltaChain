...
// deployEthstats queries the user for various input on deploying an dchstats
...
func (w *wizard) deployEthstats() {
	...
	// Retrieve any active dchstats configurations from the server
	...
	fmt.Println()
	fmt.Printf("Which port should dchstats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy dchstats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on dchstats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot dchstats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the dchstats server on the host
	nocache := false
	if existed {
		fmt.Println()
		fmt.Printf("Should the dchstats be built from scratch (y/n)? (default = no)\n")
	...
	if out, err := deployEthstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned, nocache); err != nil {
		log.Error("Failed to deploy dchstats container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
