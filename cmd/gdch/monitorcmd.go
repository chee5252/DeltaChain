...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The Gdch monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	// Attach to an Deltachain node over IPC or RPC
	endpoint := ctx.String(monitorCommandAttachFlag.Name)
	if client, err = dialRPC(endpoint); err != nil {
		utils.Fatalf("Unable to attach to gdch node: %v", err)
	}
	defer client.Close()
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gdch (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached gdch node and retrieves the entire set
