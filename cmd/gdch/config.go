...
type gdchConfig struct {
	...
	Shh       whisper.Config
	Node      node.Config
	Ethstats  dchstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *gdchConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "dch", "shh")
	cfg.WSModules = append(cfg.WSModules, "dch", "shh")
	cfg.IPCPath = "gdch.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, gdchConfig) {
	// Load defaults.
	cfg := gdchConfig{
		Eth:       dch.DefaultConfig,
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the Deltachain Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
