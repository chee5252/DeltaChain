...
Use "deltachaineum dump 0" to dump the genesis block.`,
	}
)
...
func importPreimages(ctx *cli.Context) error {
	...
	stack := makeFullNode(ctx)
	diskdb := utils.MakeChainDatabase(ctx, stack).(*dchdb.LDBDatabase)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := dchdb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*dchdb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
