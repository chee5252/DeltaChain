	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mdchod> [<args>]",
					Usage:     "call a node RPC mdchod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mdchod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mdchod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mdchod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mdchod string, args ...string) error {
	parts := strings.SplitN(mdchod, "_", 2)
	namespace := parts[0]
	mdchod = parts[1]
