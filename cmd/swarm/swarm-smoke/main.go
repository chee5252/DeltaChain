...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.BoolFlag{
			Name:        "single",
			Usage:       "whdeltachain to fetch content from a single node or from all nodes",
			Destination: &single,
		},
		cli.IntFlag{
			Name:        "track-timeout",
...
func emitMetrics(ctx *cli.Context) error {
	if gdchmetrics.Enabled {
		...
		return influxdb.InfluxDBWithTagsOnce(gdchmetrics.DefaultRegistry, endpoint, database, username, password, "swarm-smoke.", tagsMap)
	}
