...
```text
#cp $GOPATH/src/github.com/deltachaineum/go-deltachaineum/cmd/clef/4byte.json $PWD
```
...
_Note: make sure that `0x694...` is an account you have access to. You can create it either via the clef or the traditional account cli tool. If the latter was chosen, make sure both clef and gdch use the same keystore by specifing `--keystore path/to/your/keystore` when running clef._
...
```
#curl -H "Content-Type: application/json" -X POST --data "{\"jsonrpc\":\"2.0\",\"mdchod\":\"account_sign\",\"params\":[\"0x694267f14675d7e1b9494fd8d72fefe1755710fa\",\"0x$(xxd -pu <<< '  bazonk baz gaz')\"],\"id\":67}" http://localhost:8550/
...
#curl -H "Content-Type: application/json" -X POST --data "{\"jsonrpc\":\"2.0\",\"mdchod\":\"account_sign\",\"params\":[\"0x694267f14675d7e1b9494fd8d72fefe1755710fa\",\"0x$(xxd -pu <<< '  bonk baz gaz')\"],\"id\":67}" http://localhost:8550/
