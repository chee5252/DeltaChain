...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 deltachain` in value to maximum `1 deltachain` per 24h period
...
A ruleset file is implemented as a `js` file. Under the hood, the ruleset-engine is a `SignerUI`, implementing the same mdchods as the `json-rpc` mdchods
...
invokes the corresponding mdchod. In doing so, there are three possible outcomes:

1. JS returns "Approve"
  * Auto-approve request
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or somdching else is returned
...
implemented for `gdch`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whdeltachain any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
