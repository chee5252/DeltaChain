...
// faucet is a Deltachain faucet backed by a light client.
package main
...
var (
	...
	dchPortFlag = flag.Int("dchport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Deltachain protocol")
	statsFlag   = flag.String("dchstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Deltachains to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Deltachain and the faucet")
)

var (
	deltachain = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Deltachains", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *dchPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Deltachain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Deltachain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Deltachain protocol stack
	client *dchclient.Client   // Client connection to the Deltachain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Deltachain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := dch.DefaultConfig
	...
	// Assemble the dchstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightDeltachain
			ctx.Service(&serv)
			return dchstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := dchclient.NewClient(api)
...
// close terminates the Deltachain connection and tears down the faucet.
...
// apiHandler handles requests for Deltachain grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, deltachain),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Deltachain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somdching funky happened, please open an issue at https://github.com/deltachaineum/go-deltachaineum/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), deltachain)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, deltachain)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and Deltachain address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Deltachain address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Deltachain address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Deltachain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Deltachain address,
