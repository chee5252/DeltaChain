...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error:  &jsonError{Code: -32601, Message: "The mdchod no_such_mdchod_ does not exist/is not available"},
		},
	}
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("dch", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("dch", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
// This test reproduces https://github.com/deltachaineum/go-deltachaineum/issues/17837 where the
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "dch_echo", i)
			if err != nil {
				if !wantError {
...
func TestClientReconnect(t *testing.T) {
	...
	// We spawn multiple calls here to check whdeltachain this hangs somehow.
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-deltachaineum-test-ipc-%d-%d", os.Getpid(), rand.Int63())
