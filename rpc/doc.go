...
/*
Package rpc provides access to the exported mdchods of an object across a network
or other I/O connection. After creating a server instance objects can be registered,
making it visible from the outside. Exported mdchods that follow specific
conventions can be called remotely. It also has support for the publish/subscribe
pattern.

Mdchods that satisfy the following criteria are made available for remote access:
 - object must be exported
 - mdchod must be exported
 - mdchod returns 0, 1 (response or error) or 2 (response and error) values
 - mdchod argument(s) must be exported or builtin types
 - mdchod returned value(s) must be exported or builtin types
...
This RPC mdchod can be called with 2 integers and a null value as third argument.
...
arguments. It will pass the mod argument as nil to the RPC mdchod.

The server offers the ServeCodec mdchod which accepts a ServerCodec instance. It will
...
A mdchod that is considered eligible for notifications must satisfy the following criteria:
 - object must be exported
 - mdchod must be exported
 - first mdchod argument type must be context.Context
 - mdchod argument(s) must be exported or builtin types
 - mdchod must return the tuple Subscription, error
