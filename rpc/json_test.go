...
func TestJSONRequestParsing(t *testing.T) {
	...
	req := bytes.NewBufferString(`{"id": 1234, "jsonrpc": "2.0", "mdchod": "calc_add", "params": [11, 22]}`)
	var str string
	...
	if requests[0].mdchod != "add" {
		t.Fatalf("Expected mdchod 'Add' but got '%s'", requests[0].mdchod)
	}
