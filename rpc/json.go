...
const (
	jsonrpcVersion           = "2.0"
	serviceMdchodSeparator   = "_"
	subscribeMdchodSuffix    = "_subscribe"
	unsubscribeMdchodSuffix  = "_unsubscribe"
	notificationMdchodSuffix = "_subscription"
)

type jsonRequest struct {
	Mdchod  string          `json:"mdchod"`
...
// on explicitly given encoding and decoding mdchods.
...
// checkReqId returns an error when the given reqId isn't valid for RPC mdchod calls.
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	// subscribe are special, they will always use `subscribeMdchod` as first param in the payload
	if strings.HasSuffix(in.Mdchod, subscribeMdchodSuffix) {
		reqs := []rpcRequest{{id: &in.Id, isPubSub: true}}
		if len(in.Payload) > 0 {
			// first param must be subscription name
			var subscribeMdchod [1]string
			if err := json.Unmarshal(in.Payload, &subscribeMdchod); err != nil {
				log.Debug(fmt.Sprintf("Unable to parse subscription mdchod: %v\n", err))
				return nil, false, &invalidRequestError{"Unable to parse subscription request"}
			}

			reqs[0].service, reqs[0].mdchod = strings.TrimSuffix(in.Mdchod, subscribeMdchodSuffix), subscribeMdchod[0]
	...
	if strings.HasSuffix(in.Mdchod, unsubscribeMdchodSuffix) {
		return []rpcRequest{{id: &in.Id, isPubSub: true,
			mdchod: in.Mdchod, params: in.Payload}}, false, nil
	}

	elems := strings.Split(in.Mdchod, serviceMdchodSeparator)
	if len(elems) != 2 {
		return nil, false, &mdchodNotFoundError{in.Mdchod, ""}
	}

	// regular RPC call
	if len(in.Payload) == 0 {
		return []rpcRequest{{service: elems[0], mdchod: elems[1], id: &in.Id}}, false, nil
	}

	return []rpcRequest{{service: elems[0], mdchod: elems[1], id: &in.Id, params: in.Payload}}, false, nil
}
...
func parseBatchRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	for i, r := range in {
		...
		// subscribe are special, they will always use `subscriptionMdchod` as first param in the payload
		if strings.HasSuffix(r.Mdchod, subscribeMdchodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true}
			if len(r.Payload) > 0 {
				// first param must be subscription name
				var subscribeMdchod [1]string
				if err := json.Unmarshal(r.Payload, &subscribeMdchod); err != nil {
					log.Debug(fmt.Sprintf("Unable to parse subscription mdchod: %v\n", err))
					return nil, false, &invalidRequestError{"Unable to parse subscription request"}
				}

				requests[i].service, requests[i].mdchod = strings.TrimSuffix(r.Mdchod, subscribeMdchodSuffix), subscribeMdchod[0]
		...
		if strings.HasSuffix(r.Mdchod, unsubscribeMdchodSuffix) {
			requests[i] = rpcRequest{id: id, isPubSub: true, mdchod: r.Mdchod, params: r.Payload}
			continue
		}
		...
		if elem := strings.Split(r.Mdchod, serviceMdchodSeparator); len(elem) == 2 {
			requests[i].service, requests[i].mdchod = elem[0], elem[1]
		} else {
			requests[i].err = &mdchodNotFoundError{r.Mdchod, ""}
		}
	}
...
func (c *jsonCodec) CreateNotification(subid, namespace string, event interface{}) interface{} {
	return &jsonNotification{Version: jsonrpcVersion, Mdchod: namespace + notificationMdchodSuffix,
