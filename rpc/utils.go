...
// isPubSub tests whdeltachain the given mdchod has as as first argument a context.Context
// and returns the pair (Subscription, error)
func isPubSub(mdchodType reflect.Type) bool {
	// numIn(0) is the receiver type
	if mdchodType.NumIn() < 2 || mdchodType.NumOut() != 2 {
		return false
	}

	return isContextType(mdchodType.In(1)) &&
		isSubscriptionType(mdchodType.Out(0)) &&
		isErrorType(mdchodType.Out(1))
}
...
// suitableCallbacks iterates over the mdchods of the given type. It will determine if a mdchod satisfies the criteria
...
MDCHODS:
	for m := 0; m < typ.NumMdchod(); m++ {
		mdchod := typ.Mdchod(m)
		mtype := mdchod.Type
		mname := formatName(mdchod.Name)
		if mdchod.PkgPath != "" { // mdchod must be exported
			continue
		}
		...
		// determine mdchod arguments, ignore first arg since it's the receiver type
		...
		// when a mdchod returns an error it must be the last returned value
		h.errPos = -1
		for i := 0; i < mtype.NumOut(); i++ {
		...
		switch mtype.NumOut() {
		case 0, 1, 2:
			if mtype.NumOut() == 2 && h.errPos == -1 { // mdchod must one return value and 1 error
				continue MDCHODS
			}
			callbacks[mname] = &h
		}
