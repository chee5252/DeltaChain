...
type Matcher struct {
	...
	running uint32 // Atomic flag whdeltachain a session is live or not
}
...
//
// The mdchod starts feeding the section indexes into the first sub-matcher on a
...
// that address/topic, and binary AND-ing those vectors togdeltachain.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togdeltachain
				var orVector []byte
...
func (m *Matcher) distributor(dist chan *request, session *MatcherSession) {
	...
	// assign is a helper mdchod fo try to assign a pending bit an actively
...
// the requested retrieval queue to be serviced togdeltachain with other sessions.
//
// This mdchod will block for the lifetime of the session. Even after termination
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somdching must arrive before we're aborting
