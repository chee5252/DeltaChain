...
// Interpreter is used to run Deltachain based contracts and will utilise the
...
// keccakState wraps sha3.state. In addition to the usual hash mdchods, it also supports
...
type EVMInterpreter struct {
	...
	readOnly   bool   // Whdeltachain to throw on stateful modifications
	returnData []byte // Last CALL's return data for subsequent reuse
}

// NewEVMInterpreter returns a new instance of the Interpreter.
func NewEVMInterpreter(evm *EVM, cfg Config) *EVMInterpreter {
	// We use the STOP instruction whdeltachain to see
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		// cost is explicitly set so that the capture state defer mdchod can get the proper cost
