...
// Trie is a Deltachain Merkle Trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb dchdb.Putter) error
}
...
func NewDatabase(db dchdb.Database) Database {
...
func NewDatabaseWithCache(db dchdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb dchdb.Putter) error {
