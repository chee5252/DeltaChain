...
// ChainIndexerBackend defines the mdchods needed to process chain segments in
...
type ChainIndexer struct {
	chainDb  dchdb.Database      // Chain database to index the data from
	indexDb  dchdb.Database      // Prefixed table-view of the db to write index metadata into
	backend  ChainIndexerBackend // Background processor generating the index data content
	children []*ChainIndexer     // Child indexers to cascade chain updates to

	active    uint32          // Flag whdeltachain the event loop was started
...
func NewChainIndexer(chainDb, indexDb dchdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
