...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(dchdb.NewMemDatabase()))
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(common.Hash{}, state.NewDatabase(dchdb.NewMemDatabase()))
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Deltachain))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whdeltachain the pending state is in sync with the
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Deltachain))
...
// Tests that the transaction limits are enforced the same way irrelevant whdeltachain
