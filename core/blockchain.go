...
// Package core implements the Deltachain consensus protocol.
package core
...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/common/prque"
...
type CacheConfig struct {
	Disabled       bool          // Whdeltachain to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     dchdb.Database // Low level persistent database to store final content in
	...
	shouldPreserve func(*types.Block) bool // Function used to determine whdeltachain should preserve the given block.
}

// NewBlockChain returns a fully initialised block chain using information
// available in the database. It initialises the default Deltachain Validator and
// Processor.
func NewBlockChain(db dchdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
...
// loadLastState loads the last known chain state from the database. This mdchod
...
//
// This mdchod only rolls back the current block. The current header and current
...
// insert injects a new head block into the current block chain. This mdchod
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= dchdb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - dchdb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have somdching meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
	// Do a sanity check that the provided chain is actually ordered and linked
...
//
// This mdchod is split out so that import batches that require re-injecting
...
// found.
//
// The mdchod writes all (header-and-body-valid) blocks to disk, then tries to
...
//
// The verify parameter can be used to fine tune whdeltachain nonce verification
...
//
// Note: This mdchod is not concurrent-safe with inserting blocks simultaneously
