...
// mdchod to sign the transaction before submission.
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *DeltachainClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *DeltachainClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
...
func (c *BoundContract) Call(opts *CallOpts, out *Interfaces, mdchod string, args *Interfaces) error {
	if len(out.objects) == 1 {
		result := out.objects[0]
		if err := c.contract.Call(&opts.opts, result, mdchod, args.objects...); err != nil {
			return err
		}
		...
		if err := c.contract.Call(&opts.opts, &results, mdchod, args.objects...); err != nil {
			return err
		}
...
func (c *BoundContract) Transact(opts *TransactOpts, mdchod string, args *Interfaces) (tx *Transaction, _ error) {
	rawTx, err := c.contract.Transact(&opts.opts, mdchod, args.objects...)
	if err != nil {
		return nil, err
	}
