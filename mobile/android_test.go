...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gdch.LightScryptN, Gdch.LightScryptP);
		...
		try {
			...
			// data from this mdchod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.deltachaineum", new NodeConfig());
			node.start();
			...
			DeltachainClient ec = node.getDeltachainClient();
	...
	// signatures too. Regression test for go-deltachaineum issue #14599.
...
//
// This mdchod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogdeltachain
	...
	workspace, err := ioutil.TempDir("", "gdch-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/deltachaineum/gdchtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gdch and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.deltachaineum", "github.com/deltachaineum/go-deltachaineum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gdch.aar"), "gdch.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "deltachaineum", "gdchtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.deltachaineum.gdchtest"
