...
// Package gdch contains the simplified mobile APIs to go-deltachaineum.
//
// The scope of this package is *not* to allow writing a custom Deltachain client
// with pieces plucked from go-deltachaineum, rather to allow writing native dapps on
...
// from the go-deltachaineum repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-deltachaineum version.
// This is needed because gomobile cannot expose named types for now.
//
// Whenever a mdchod argument or a return type is a custom struct, the pointer
...
// a go slice with the mdchods `Size`, `Get` and `Set`. Further slice operations
...
//
// If a mdchod has multiple return values (e.g. some return + an error), those
