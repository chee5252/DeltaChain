...
# 1. install gdch for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for gdch
# 5. create gdch, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable DELTACHAIN_SOCKET
...
!define GROUPNAME "Deltachain"
!define APPNAME "Gdch"
!define DESCRIPTION "Official Go implementation of the Deltachain protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since gdch is installed global
