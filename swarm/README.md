...
[https://swarm.deltachaineum.org](https://swarm.deltachaineum.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the deltachaineum web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.
...
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/deltachainsphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/deltachaineum/go-deltachaineum

    go install github.com/deltachaineum/go-deltachaineum/cmd/swarm
...
To run Swarm you need an Deltachain account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Deltachain address representing that account. For example:
...
### Deltachain Name Service resolution

The Deltachain Name Service is the Deltachain equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.dch`), `swarm` has to connect to a `gdch` instance, which is synced with the Deltachain mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.deltachaineum/gdch.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/deltachaineum/go-deltachaineum`.

Most likely you will be working from your fork of `go-deltachaineum`, let's say from `github.com/nirname/go-deltachaineum`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-deltachaineum.git $GOPATH/src/github.com/deltachaineum/go-deltachaineum
```
...
Testing only some mdchods:

```
go test -v -cpu 4 ./dch -run TestMdchod
```

Note: here all tests with prefix TestMdchod will be run, so if you got TestMdchod, TestMdchod1, then both!
...
We have built a tool to help with automatic start of Grafana and InfluxDB and provisioning of dashboards at https://github.com/nonsense/statdch , which requires that you have Docker installed.

Once you have `statdch` installed, and you have Docker running locally, you have to:

1. Run `statdch` and keep it running in the background
```
statdch --rm --grafana-dashboards-folder $GOPATH/src/github.com/deltachaineum/go-deltachaineum/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Deltachain Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
You can find a few reference Swarm decentralised applications at: https://swarm-gateways.net/bzz:/swarmapps.dch

Their source code can be found at: https://github.com/deltachainsphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/deltachainsphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/deltachaineum/go-deltachaineum/wiki/Code-Review-Guidelines).
