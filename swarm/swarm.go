...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/contracts/chequebook"
	"github.com/deltachaineum/go-deltachaineum/contracts/ens"
	...
	"github.com/deltachaineum/go-deltachaineum/p2p/protocols"
	"github.com/deltachaineum/go-deltachaineum/params"
	"github.com/deltachaineum/go-deltachaineum/rpc"
	"github.com/deltachaineum/go-deltachaineum/swarm/api"
	httpapi "github.com/deltachaineum/go-deltachaineum/swarm/api/http"
	"github.com/deltachaineum/go-deltachaineum/swarm/fuse"
	"github.com/deltachaineum/go-deltachaineum/swarm/log"
	"github.com/deltachaineum/go-deltachaineum/swarm/network"
	"github.com/deltachaineum/go-deltachaineum/swarm/network/stream"
	"github.com/deltachaineum/go-deltachaineum/swarm/pss"
	"github.com/deltachaineum/go-deltachaineum/swarm/state"
	"github.com/deltachaineum/go-deltachaineum/swarm/storage"
	"github.com/deltachaineum/go-deltachaineum/swarm/storage/feed"
	"github.com/deltachaineum/go-deltachaineum/swarm/storage/mock"
	"github.com/deltachaineum/go-deltachaineum/swarm/swap"
	"github.com/deltachaineum/go-deltachaineum/swarm/tracing"
)
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = dchclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	dchClient := dchclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, dchClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := dchclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
