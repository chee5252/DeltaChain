...
#
# The cluster contains a bootnode, a gdch node and multiple swarm nodes, with
...
# static gdch configuration
GDCH_IP="192.168.33.3"
GDCH_RPC_PORT="8545"
GDCH_RPC_URL="http://${GDCH_IP}:${GDCH_RPC_PORT}"
...
# network namespaces togdeltachain
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the gdch and
# swarm nodes
start_bootnode() {
...
# start_gdch_node starts a gdch node with --datadir pointing at <base-dir>/gdch
# and a single, unlocked account with password "gdch"
start_gdch_node() {
  local dir="${base_dir}/gdch"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gdch account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GDCH_IP}"
    --rpcport   "${GDCH_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gdch" "${GDCH_IP}" "$(which gdch)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vdch pair
  local vdch0="vdch${name}0"
  local vdch1="vdch${name}1"
  ip link add name "${vdch0}" type vdch peer name "${vdch1}"

  # add one end to the bridge
  ip link set dev "${vdch0}" master "${BRIDGE_NAME}"
  ip link set dev "${vdch0}" up

  # add the other end to the namespace, rename it dch0 and give it the ip
  ip link set dev "${vdch1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vdch1}" name "dch0"
  ip netns exec "${name}" ip link set dev "dch0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "dch0"
}
...
create_account() {
  ...
  gdch --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
