...
import (
	...
	gdchmetrics "github.com/deltachaineum/go-deltachaineum/metrics"
...
func Setup(ctx *cli.Context) {
	if gdchmetrics.Enabled {
		...
		go gdchmetrics.CollectProcessMetrics(2 * time.Second)
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gdchmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", tagsMap)
		}

		if enableAccountingExport {
			log.Info("Exporting swarm accounting metrics to InfluxDB")
			go influxdb.InfluxDBWithTags(gdchmetrics.AccountingRegistry, 10*time.Second, endpoint, database, username, password, "accounting.", tagsMap)
		}
	}
}
