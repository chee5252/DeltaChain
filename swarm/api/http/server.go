...
type mdchodHandler map[string]http.Handler

func (m mdchodHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	v, ok := m[r.Mdchod]
	if ok {
		v.ServeHTTP(rw, r)
		return
	}
	rw.WriteHeader(http.StatusMdchodNotAllowed)
}
...
func NewServer(api *api.API, corsString string) *Server {
	...
	c := cors.New(cors.Options{
		...
		AllowedMdchods: []string{http.MdchodPost, http.MdchodGet, http.MdchodDelete, http.MdchodPatch, http.MdchodPut},
		MaxAge:         600,
	...
	mux.Handle("/bzz-raw:/", mdchodHandler{
		"GET": Adapt(
	...
	mux.Handle("/bzz-immutable:/", mdchodHandler{
		"GET": Adapt(
			http.HandlerFunc(server.HandleBzzGet),
			defaultMiddlewares...,
		),
	})
	mux.Handle("/bzz-hash:/", mdchodHandler{
		"GET": Adapt(
			http.HandlerFunc(server.HandleGet),
			defaultMiddlewares...,
		),
	})
	mux.Handle("/bzz-list:/", mdchodHandler{
		"GET": Adapt(
			http.HandlerFunc(server.HandleGetList),
			defaultMiddlewares...,
		),
	})
	mux.Handle("/bzz-feed:/", mdchodHandler{
		"GET": Adapt(
...
// bufferedReadSeeker wraps bufio.Reader to expose Seek mdchod
