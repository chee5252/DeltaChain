...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/swarm/storage/feed/lookup"
	...
	swarm "github.com/deltachaineum/go-deltachaineum/swarm/api/client"
	"github.com/deltachaineum/go-deltachaineum/swarm/storage"
	"github.com/deltachaineum/go-deltachaineum/swarm/storage/feed"
	"github.com/deltachaineum/go-deltachaineum/swarm/testutil"
)
...
func TestBzzWithFeed(t *testing.T) {
	...
	// put togdeltachain some data for our test:
	dataBytes := []byte(`
	//
...
// Test Swarm feeds using the raw update mdchods
func TestBzzFeed(t *testing.T) {
	...
	topic, _ := feed.NewTopic("foo.dch", nil)
	updateRequest := feed.NewFirstRequest(topic)
...
func testBzzGetPath(encrypted bool, t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run("html list "+c.path, func(t *testing.T) {
			req, err := http.NewRequest(http.MdchodGet, url, nil)
			if err != nil {
...
func TestMdchodsNotAllowed(t *testing.T) {
	srv := NewTestSwarmServer(t, serverFunc, nil)
	defer srv.Close()
	databytes := "bar"
	for _, c := range []struct {
	...
	}{
		{
			...
			code: http.StatusMdchodNotAllowed,
		}, {
...
func httpDo(httpMdchod string, url string, reqBody io.Reader, headers map[string]string, verbose bool, t *testing.T) (*http.Response, string) {
	// Build the Request
	req, err := http.NewRequest(httpMdchod, url, reqBody)
	if err != nil {
		t.Fatal(err)
	}
	...
	if verbose {
		t.Log(req.Mdchod, req.URL, req.Header, req.Body)
	}
...
func TestGet(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run("GET "+testCase.uri, func(t *testing.T) {
			res, body := httpDo(testCase.mdchod, testCase.uri, nil, testCase.headers, testCase.verbose, t)
...
func TestModify(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			mdchod:             "PUT",
			headers:            map[string]string{},
			expectedStatusCode: http.StatusMdchodNotAllowed,
			verbose:            false,
		},
		{
		...
		t.Run(testCase.mdchod+" "+testCase.uri, func(t *testing.T) {
			reqBody := bytes.NewReader(testCase.requestBody)
			res, body := httpDo(testCase.mdchod, testCase.uri, reqBody, testCase.headers, testCase.verbose, t)
...
func TestMultiPartUpload(t *testing.T) {
	...
	file2.Write([]byte("imagindchisisjpegdata"))
	form.Close()
