...
import (
	...
	"github.com/deltachaineum/go-deltachaineum/swarm/sctx"
	"github.com/deltachaineum/go-deltachaineum/swarm/spancontext"
...
func SetRequestID(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterCounter(fmt.Sprintf("http.request.%s", r.Mdchod), nil).Inc(1)
		log.Info("created ruid for request", "ruid", GetRUID(r.Context()), "mdchod", r.Mdchod, "url", r.RequestURI)
...
func ParseURI(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Debug("parsed request path", "ruid", GetRUID(r.Context()), "mdchod", r.Mdchod, "uri.Addr", uri.Addr, "uri.Path", uri.Path, "uri.Scheme", uri.Scheme)
...
func InitLoggingResponseWriter(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.time", r.Mdchod), nil).Update(ts)
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.%d.time", r.Mdchod, writer.statusCode), nil).Update(ts)
	})
}
...
func InstrumentOpenTracing(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if uri == nil || r.Mdchod == "" || (uri != nil && uri.Scheme == "") {
			h.ServeHTTP(w, r) // soft fail
			return
		}
		spanName := fmt.Sprintf("http.%s.%s", r.Mdchod, uri.Scheme)
