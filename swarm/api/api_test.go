...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.dch"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.dch"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.dch"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	dchAddr := "swarm.dch"
	dchHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	dchResolve := newTestResolveValidator(dchHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(dchResolve, "")),
			addr:   dchAddr,
			result: dchHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(dchResolve, ""),
				MultiResolverOptionWithResolver(dchResolve, ""),
			),
			addr:   dchAddr,
			result: dchHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(dchResolve, "dch"),
			),
			addr:   dchAddr,
			result: dchHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "dch"),
				MultiResolverOptionWithResolver(doesntResolve, "dch"),
				MultiResolverOptionWithResolver(dchResolve, "dch"),
			),
			addr:   dchAddr,
		...
		},
		{
			...
			addr:   dchAddr,
			result: dchHash,
			err:    errors.New(`DNS name not found: "swarm.dch"`),
		},
		{
