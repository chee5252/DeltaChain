...
// giving access to pss mdchods from a different process
type Client struct {
	BaseAddrHex string
...
// run prior to adding the peer, or this mdchod will
// return an error.
//
// The key must exist in the key store of the pss node
// before the peer is added. The mdchod will return an error
// if it is not.
