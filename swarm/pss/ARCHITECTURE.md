...
For the current state and roadmap of pss development please see https://github.com/deltachainsphere/swarm/wiki/swarm-dev-progress.

Please report issues on https://github.com/deltachainsphere/go-deltachaineum

Feel free to ask questions in https://gitter.im/deltachainsphere/pss

## STATUS OF THIS DOCUMENT

`pss` is under active development, and the first implementation is yet to be merged to the Deltachain main branch. Expect things to change.
...
This "topic" is not like the subject of an email message, but a hash-like arbitrary 4 byte value. A valid topic can be generated using the `pss_*ToTopic` API mdchods.
...
Since pss itself never requires a confirmation from a peer of whdeltachain a message is received or not, one could argue that pss shows `UDP`-like behavior.
...
The API mdchods `pss_sendSym` and `pss_sendAsym` sends an arbitrary byte slice with a specific topic to a pss peer using the respective encryption scheme. The key passed to the send mdchod must be associated with a topic in the pss key store prior to sending, or the send mdchod will fail.

Return values from the send mdchods do *not* indicate whdeltachain the message was successfully delivered to the pss peer. It *only* indicates whdeltachain or not the message could be passed on to the network. If the message could not be forwarded to any peers, the mdchod will fail.
...
Handshakes are activated in the code implementation of the node by running `SetHandshakeController()` on the pss node instance BEFORE starting the node service. The mdchods exposed by the HandshakeController's API gives the possibility to initiate, remove and check the state of handshakes and associated keys.
...
In order to message a peer using this layer, a `Protocol` object must first be instantiated. When this is done, peers can be added using the protocol's `AddPeer()` mdchod. The peer's key/topic combination must be in the pss key store before the peer can be aded.
...
As with the `AddPeer()` mdchod, the key/topic of the originating peer must exist in the pss key store.

#### TOPICS IN DEVP2P

The `ProtocolTopic()` mdchod should be used to determine the correct topic to use for a pss `Protocol` instance.
...
When processing an incoming message, `pss` detects whdeltachain it is encrypted symmetrically or asymmetrically.
...
Under the hood, pss implements its own MsgReadWriter, which bridges MsgReadWriter.WriteMsg with Pss.SendRaw, and deftly adds an InjectMsg mdchod which pipes incoming messages to appear on the MsgReadWriter.ReadMsg channel.
