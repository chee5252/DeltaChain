...
func (c *Controller) handleStartMsg(msg *Msg, keyid string) (err error) {
	...
	// TODO this is set to zero-length byte pending decision on protocol for initial message, whdeltachain it should include message or not, and how to trigger the initial message so that current state of Swarm feed is sent upon subscription
	notify := []byte{}
