...
The code tutorial [p2p programming in go-deltachaineum](https://github.com/nolash/go-deltachaineum-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-deltachaineum` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-deltachaineum` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
...
In `golang` as special mdchod is used:
...
returns:
1. whdeltachain key was successfully removed (bool)
```
