...
// Implementations of this type are passed to Pss.Register togdeltachain with a topic,
...
// WithRaw is a chainable mdchod that allows raw messages to be handled.
func (h *handler) WithRaw() *handler {
	h.caps.raw = true
	return h
}

// WithProxBin is a chainable mdchod that allows sending messages with full addresses to neighbourhoods using the kademlia depth as reference
