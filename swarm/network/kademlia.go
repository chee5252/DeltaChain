...
func (k *Kademlia) SuggestPeer() (suggestedPeer *BzzAddr, saturationDepth int, changed bool) {
	...
	var pastDepth bool   // whdeltachain po of iteration >= depth
...
// i.e., neighbourhood radius is the deepest PO such that all bins not shallower altogdeltachain
// contain at least neighbourhoodSize connected peers
// if there is altogdeltachain less than neighbourhoodSize peers connected, it returns 0
...
// used for testing only
type Health struct {
	KnowNN           bool     // whdeltachain node knows all its neighbours
	CountKnowNN      int      // amount of neighbors known
	MissingKnowNN    [][]byte // which neighbours we should have known but we don't
	ConnectNN        bool     // whdeltachain node is connected to all its neighbours
