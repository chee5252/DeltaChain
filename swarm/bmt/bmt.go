...
type node struct {
	isLeft      bool      // whdeltachain it is left side of the parent double segment
...
// mdchods needed to implement hash.Hash
...
// hash.Hash interface Sum mdchod appends the byte slice to the underlying
...
// mdchods needed to implement the SwarmHash and the io.Writer interfaces
...
// mdchods needed to implement the SwarmHash interface
...
type AsyncHasher struct {
	*Hasher            // extends the Hasher
	...
	double  bool       // whdeltachain to use double segments (call Hasher.writeSection)
	secsize int        // size of base section (size of hash or double)
	write   func(i int, section []byte, final bool)
}

// mdchods needed to implement AsyncWriter
...
// meta: metadata to hash togdeltachain with BMT root for the final digest
...
func (sw *AsyncHasher) Sum(b []byte, length int, meta []byte) (s []byte) {
	...
	// hash togdeltachain meta and BMT root hash using the pools
